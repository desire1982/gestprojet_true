...
// Connaitre le nombre de personne connecté
$req_personne_connecte="SELECT COUNT(*) AS Nombrepersonne FROM `tbl_user` WHERE `tbl_user`.`connecte` = 1";
$resNonmbrepersonne=mysql_query($req_personne_connecte);
$Totalpersonneconnecte= mysql_fetch_array($resNonmbrepersonne);
$NombresConnectes= $Totalpersonneconnecte['Nombrepersonne'];

// Connaitre la liste des personnes connectées
$req_liste_pers ="SELECT `tbl_user`.`user_name` AS nom_utilisateur , `tbl_user`.`date_dernier_connexion` AS connecte_le FROM `tbl_user` WHERE `tbl_user`.`connecte` = 1
GROUP BY `tbl_user`.`user_name`";
 $res_liste_pers=mysql_query("$req_liste_pers");


//Requete pour connaitre le nombre de doublon des destinations

$req_doublon_dest="SELECT `tbl_destination`.`destination` AS destination, COUNT(*) AS Nombre_destination
                    ...
                    <div class="panel panel-yellow">
                        <div class="panel-heading">
                            <div class="row">
                                ...
                                <div class="col-xs-9 text-right">
                                    <div class="huge" style="font-size:16px; font-weight:bold; color:#000"><?php echo $NombresConnectes; ?></div>
                                    <div>Nombre de connecté</div>
                                </div>
                            </div>
                        </div>
                        <a href="#">
...
<!--  Recherche de doublon  -->
  ...
  <div class="table-responsive">
                                        <table class="table table-bordered table-hover table-striped">
                                            <thead>
                                            <tr>
                                                    <th colspan="2" align="center" style="text-align:center">DOUBLON</th>
                                                    
                                                </tr>
                                                <tr>
                                                    <th>destination</th>
                                                    <th>Nombre de doublon</th>
                                                </tr>
                                            </thead>
                                            
               <?php 
			   while ($donnee = mysql_fetch_array($res_doublon_dest)){
			    
				?>                             
                                            <tbody>
                                                <tr>
                                                    <td><?php echo $donnee['destination'];?></td>
                                                    <td><?php echo $donnee['Nombre_destination'];?></td>
