...
if (!empty($_POST) ) {
    ...
    define("USER", "root");
    define("PASSWORD","");
    define("DB", "gestprojet");
    define("MyTable", "tbl_dotation_projet");
    $connection = mysql_connect(HOST, USER, PASSWORD, DB) OR DIE("Impossible to access to DB : " . mysql_error());
    ...
    function getData($sql){
      //  global $connection ;//we use connection already opened
        $query = mysql_query($sql) OR DIE ("Can't get Data from DB , check your SQL Query " );
        $data = array();
	...
	//print_r(getData("SELECT * FROM tbl_dotation_projet"));
  ...
  echo  $recordsTotal;
   ...
   // print_r(getData("SELECT * FROM tbl_dotation_projet"));//Result
